...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self prefixString] length] != 0 &&
        _text.length > 3+self.prefixString.length+1) {
        [attString addAttribute:(NSString*)(kCTForegroundColorAttributeName)
                          value:(id)[MAUICatalog redColor].CGColor
                          range:NSMakeRange(2, [[self prefixString] length]+1)];
    }
...
//                      range:NSMakeRange(0, attString.length)];
    ...
    float drawLineX = 0;
    float drawLineY = self.bounds.origin.y + self.frame.size.height - self.font.ascender;
    BOOL drawFlag = YES;
...
- (NSMutableArray *)runsArray {
    ...
    result = [TQRichTextEmojiRun analyzeText:_text runsArray:&array];
    
    result = [TQRichTextURLRun analyzeText:_text runsArray:&array];
    return array;
}
